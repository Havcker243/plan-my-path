-- ============================================================================
-- COMPLETE REQUIREMENTS SYSTEM SETUP
...
-- ============================================================================
-- STEP 3: INSERT REQUIREMENT GROUPS
-- ============================================================================

-- Get the CS major ID (we'll use it in all inserts)
DO $$
DECLARE
    cs_major_id UUID;
    core_fixed_id UUID;
...
BEGIN
    ...
    INSERT INTO requirement_groups (major_id, group_id, group_name, group_type, credits_required_min, credits_required_max, description, display_order)
    VALUES (cs_major_id, 'foreign_language', 'Foreign Language', 'all_of', 6, 6, 'Elementary foreign language I and II', 6)
    ON CONFLICT (major_id, group_id) DO UPDATE SET updated_at = NOW()
    ...
    -- Insert Cognates
    INSERT INTO requirement_groups (major_id, group_id, group_name, group_type, credits_required_min, credits_required_max, description, display_order)
    VALUES (cs_major_id, 'cognates', 'Required Cognates', 'all_of', 17, 17, 'Mathematics and statistics courses', 7)
    ON CONFLICT (major_id, group_id) DO UPDATE SET updated_at = NOW()
    ...
    INSERT INTO requirement_groups (major_id, group_id, group_name, group_type, credits_required_min, credits_required_max, description, display_order)
    VALUES (cs_major_id, 'major_electives', 'Major Required Electives', 'credit_threshold', 6, 8, 'CSCI courses at 200 level or above', 9)
    ON CONFLICT (major_id, group_id) DO UPDATE SET updated_at = NOW()
    RETURNING id INTO major_elec_id;

    -- Insert rule for major electives
...
-- ============================================================================
-- STEP 4: VERIFY DATA - Run these queries to see your data
-- ============================================================================

-- View all majors
SELECT * FROM majors;

-- View all requirement groups for CS
SELECT
    rg.group_name,
    rg.group_type,
...
-- View all required courses for CS Major Requirements
SELECT
    rg.group_name,
    rc.course_code,
    rc.course_name,
...
JOIN requirement_groups rg ON rg.id = rc.group_id
JOIN majors m ON m.id = rg.major_id
WHERE m.code = 'CSCI' AND rg.group_id = 'core_group_e'
ORDER BY rc.course_code;

-- View major electives rule
SELECT
    rg.group_name,
    rr.subject_code,
    rr.min_level,
...
JOIN majors m ON m.id = rg.major_id
WHERE m.code = 'CSCI' AND rg.group_id = 'major_electives';
